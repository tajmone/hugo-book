...
// @FOOTNOTE IN TABLE: All footnotes occur inside inside cells.

// @FOOTNOTE ADDED: (Issue #64)
//    Footnotes fn1 and fn2 were added to the original.
//    Based on Roody Yogurt's annotated PDF.

:fn1: pass:q[footnote:[`INDENT_SIZE` is actually a constant, to change its value you must define it before including *hugolib.h*.]]

:fn2: pass:q[footnote:[`AFTER_PERIOD` is actually a constant, to change its value you must define it before including *hugolib.h*.]]

:fn3: footnote:[Used generally for room objects in order to give a grammatically correct description if necessary; also for containers and platforms.]

// @FOOTNOTE ISSUE: In HTML output, the text of this note is placed at the bottom
//    of the cell instead of the end of the document, due to the cell being
//    styled as 'a' (AsciiDoc). This is due to a limitation in Asciidoctor, which
//    should be fixed in the future (see Issue #22).

:fn4: pass:q[footnote:[A "`grandchild`" of an object is a child of a child of a given parent object, or a child object thereof, recursively searched.]]
...
| INDENT_SIZE{wj}{fn1}  | for paragraph indenting
| AFTER_PERIOD{wj}{fn2} | string of spaces following a full-stop
...
// @FOOTNOTE HACK: The cell is styled as 'd' (default) so that the footnote text
//                 will be displayed at the end of the document (in HTML docs).

| preposition (prep)
d| "`in`", "`inside`", "`outside of`", etc.{wj}{fn3}
...
Returns `object` if the specified object is present as a possession of the specified parent, even as a grandchild,{wj}{fn4} otherwise returns false.
...
// @EDITED TEXT: (documented & waiting approval: Issues #65 and #67)
//    - text# --> textdata#
//    - The whole definition was amended to mirror current Hugo behaviour.

| 47 | [.big.red]##textdata&num;## | *INTERNAL STATEMENT*

    47 <offset> <text address>

Where `<text{nbsp}address>` is two bytes (in lowest-to-highest byte order) giving the address of the entry in the text bank at `<offset>` (one byte).
