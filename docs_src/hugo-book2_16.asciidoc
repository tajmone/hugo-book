...
See <<hugo-book2_03#15-2_data_types>>, which describes all the valid types of values.
...
TIP: See <<hugo-book1#book1>> for further elaboration on the `system` statement.
...
// @XREF: "The Hugo Programming Manual"
TIP: See <<hugo-book1#book1>> for further elaboration on the `window` statement.
