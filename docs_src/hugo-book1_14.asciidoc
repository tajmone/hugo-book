...
Sets the current font attributes as specified by `bitmask`, where `bitmask` is one or more font-style constants (see <<constants_2,library constants>>, above) combined with `{vbar}` or `+`.
