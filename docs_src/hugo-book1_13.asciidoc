...
// @XREF ADD: page 64 (Table of Hugo Lib Colors in "§4.2. Constants")
// (could use Table Caption here!)
...
// @TEXT TYPO: "both TEST strings" --> "both TEXT strings"?
where `<output>` can consist of both test strings enclosed in quotation marks (`"..."`), and values representing dictionary addresses, such as object names.
